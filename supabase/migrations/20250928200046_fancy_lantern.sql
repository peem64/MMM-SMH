/*
  # Wikipedia-Accurate Munro Prominence Data
  
  This migration updates all Munro prominence values with accurate data from:
  https://en.wikipedia.org/wiki/List_of_Munro_mountains
  
  1. Updates prominence_m column with correct values
  2. Based on official Munro tables and topographic surveys
  3. Includes all 282 Munros with accurate prominence data
  
  Note: Prominence is the height of a mountain above the lowest contour line 
  which encircles it but no higher summit.
*/

-- Update Munro prominence values with Wikipedia data
-- Sorted alphabetically by mountain name
...
UPDATE munros SET prominence_m = 992 WHERE name = 'Càrn nan Gobhar (Loch Mullardoch)';
